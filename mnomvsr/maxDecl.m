...
%		rmpath('../mnouvsr');
%
function [solution fsolution e k tf alpha] = maxDecl(fname, gfname, x0, epsilon_f, epsilon_x, k_max, fminname)
	tic();
	x(:, 1) = x0;
	gfx = feval(gfname, x0);
	k = 1;
	while norm(gfx) > epsilon_f
		alpha(k) = feval(fminname, @(alpha) f_alpha(fname, x(:, k), alpha, -gfx), 0, 1);
